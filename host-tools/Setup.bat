...
if not exist "%SERVEDIR%\index.html" (
    echo ERROR: dashboard\index.html not found at the delivery root "%ROOT%".
...
"!PY!" "%HOSTTOOLS%\write_config.py" --file "%SERVEDIR%\config.json" --captures-root "!CAPTURES_ROOT!"
if errorlevel 1 (
    echo       ERROR: could not write "%SERVEDIR%\config.json".
    echo       The dashboard fetches ./config.json from the folder it is served out of. Without it
    echo       the page loads and cannot connect to the game.
    echo.
    pause
    exit /b 1
)
...
echo.
echo Verifying the dashboard can actually fetch its config over HTTP...
"!PY!" "%HOSTTOOLS%\serve_dashboard.py" --directory "%SERVEDIR%" --verify-only
if errorlevel 1 (
    echo       ERROR: config.json exists on disk but is NOT fetchable from the served root.
    echo       A served-root relative path can fail in ways a file-exists check cannot see,
    echo       so setup stops here rather than leaving it for you to discover in the browser.
    echo.
    pause
    exit /b 1
)
